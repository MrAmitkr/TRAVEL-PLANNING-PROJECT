onload = function () {
    ...
    // this creates random nodes and edges(not the adjacency list)
    function createData(){
            ...
            if(n1!==n2){
                ...
                for(let j=0;j<edges.length;j++){
                    if(edges[j]['from']===n1 && edges[j]['to']===n2) {
                        if(edges[j]['type']===0)
                            works = 1; // A bus type edge already exists
                        else
                            works = 2; // A plane type edge already exists
                    }
                }
                ...
                if(works <= 1) {
                    if (works === 0 && i < V / 4) // for simplicity, one-fourth nodes can have flight as well as bus routes
                     {
                        // Adding a bus
                        edges.push({
                            type: 0,
    ...
    // creates the adjacency list out of the data{nodes, edges} generated by createData()
    function createGraph(data){
        ...
        for(let i=0;i<data['edges'].length;i++) {
            ...
            if(edge['type']===1) // plane edges nothing to do with dijsktra
                continue;
    ...
    function shouldTakePlane(edges, dist1, dist2, mn_dist) {
        ...
        return {plane, p1, p2};// cost of that plane, src---> p1(bus) and p1---->p2(plane) and p2---->des(bus)
    }
